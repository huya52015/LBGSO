...
export interface ScheduleItem {
  ...
  teacherName?: string;
  className: string;
  actGV?: string; // Hoạt động giáo viên cụ thể từ KHDH
  actHS?: string; // Hoạt động học sinh cụ thể từ KHDH
}
...
export interface LessonPlan {
  ...
  orderInLBG?: number; // Số thứ tự dòng trong Lịch báo giảng (để đồng bộ 100% thứ tự)
  grade: Grade;
  subject: string;
  subSubject?: string;
  session?: SessionType;
  ...
  actGV?: string; // Hoạt động giáo viên chi tiết từ KHDH
  actHS?: string; // Hoạt động học sinh chi tiết từ KHDH
