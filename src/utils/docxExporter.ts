...
function createActivityCellParagraphs(
  ...
  lines.forEach((line) => {
    ...
    const isVocabItem = /^\d+\.\s+[a-zA-Z]/.test(trimmed) || trimmed.startsWith("• ");
...
export async function exportTeacherTimetableDocx(
  ...
  const isSpecialist = matchedTeacher?.type === "specialist" || (schoolInfo.teacherType === "specialist" && schoolInfo.teacherName === teacherName);
  const teacherRole = matchedTeacher?.role || (isSpecialist ? `GV Chuyên ${schoolInfo.specialistSubject}` : `GVCN Lớp ${schoolInfo.className}`);
  ...
  const isSpecialistSubjectVal = (v: string) => {
    const clean = v.trim();
    if (/\((p|t|th|h|tr|q|tb|thy|tâm|phước|phương|thịnh|nhàn)\)/i.test(clean)) return true;
    const lower = clean.toLowerCase();
    if (lower === "tin học" || lower === "th" || lower === "t.học") return true;
    if (lower === "mt" || lower === "mĩ thuật" || lower === "bdmt") return true;
    if (lower === "an" || lower === "âm nhạc" || lower === "bdan") return true;
    if (lower === "ta" || lower === "tiếng anh") return true;
    if (lower === "gdtc" || lower === "bdnk" || lower === "bđnk") return true;
    return false;
  };
  ...
  const getTeacherClassesForSlot = (slotKey: string, day: string, session: string, period: number) => {
    ...
    masterTimetable.classes.forEach((cls) => {
      ...
      if (isSpecialist) {
        if (isSlotMatchingTeacherOrSubject(val, teacherName, specialistSubject)) {
          taught.push({ cls, sub: val });
        }
      } else {
        // Homeroom teacher (GVCN)
        const isAssigned =
          matchedTeacher?.assignedClasses?.includes(cls) ||
          (schoolInfo.className === cls && (schoolInfo.teacherName === teacherName || teacherName.includes("Oanh"))) ||
          (teacherName.includes("Oanh") && cls === "1A1") ||
          (teacherName.includes("Diễm") && cls === "1B") ||
          (teacherName.includes("Minh") && cls === "2A1") ||
          (teacherName.includes("Hoa") && cls === "2A") ||
          (teacherName.includes("Loan") && !teacherName.includes("T.Loan") && cls === "3A1") ||
          (teacherName.includes("Thương") && cls === "3A2") ||
          (teacherName.includes("Thiết") && cls === "3B") ||
          (teacherName.includes("Lượng") && cls === "4A1") ||
          (teacherName.includes("Tam") && cls === "4A2") ||
          (teacherName.includes("Vân") && cls === "4B") ||
          (teacherName.includes("Ngân") && cls === "5A1") ||
          (teacherName.includes("Hồng") && cls === "5A2") ||
          (teacherName.includes("T.Loan") && cls === "5B");

        if (isAssigned && !isSpecialistSubjectVal(val)) {
          taught.push({ cls, sub: val });
        }
      }
    });
  ...
  sortedPlans.forEach((plan, planIdx) => {
    ...
    docChildren.push(
      new Paragraph({
        spacing: { after: 60 },
        children: [
          new TextRun({ text: `BÀI HỌC: `, bold: true, font, size: baseSize }),
          new TextRun({ text: plan.lessonTitle, bold: true, font, size: baseSize }),
        ],
      })
    );
...
export async function exportWeeklyKHBDWithLBGFirstPageDocx(
  ...
  weekDays.forEach((day, dayIndex) => {
    ...
    plansForDay.forEach((plan, planIdxInDay) => {
      ...
      // Lesson Header with TKB Period and PPCT
      const sessionText = plan.session ? `${plan.session} Tiết ${plan.timetablePeriod || plan.periodNumber}` : `Tiết ${plan.periodNumber}`;
      docChildren.push(
        new Paragraph({
          spacing: { before: 80, after: 40 },
          children: [
            new TextRun({
              text: `Tiết ${plan.periodNumber} (TKB: ${sessionText})  |  MÔN: ${plan.subject.toUpperCase()}${plan.subSubject ? ` (${plan.subSubject.toUpperCase()})` : ""} - TIẾT PPCT: ${plan.curriculumPeriod || 1}${plan.className ? ` [Lớp ${plan.className}]` : ""}`,
              bold: true,
              color: "1E3A8A",
              font,
              size: subTitleSize,
