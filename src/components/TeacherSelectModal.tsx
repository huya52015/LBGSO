  ...
  const handleDownloadTeacherTKB = async (teacher: TeacherInfo) => {
    ...
    try {
      if (teacher.type === "specialist") {
        ...
        const cls = teacher.assignedClasses?.[0] || "1A1";
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b-2 border-black pb-1.5">
                <h4 className="font-sans font-bold text-xs uppercase tracking-wider text-black flex items-center gap-2">
                  ...
                  Khối 1 Đến Khối 5 — 10 Giáo Viên Chủ Nhiệm (GVCN)
                </h4>
                <span className="text-[10px] font-mono font-bold bg-stone-200 px-2 py-0.5 border border-stone-300 text-stone-800">
                  Phụ trách soạn KHBD Toán, TV, KH/TNXH, LS&ĐL, ĐĐ, HĐTN, CN (~23 tiết/tuần)
                </span>
              </div>
                      ...
                      <div className="mt-3.5 pt-3 border-t border-dashed border-stone-300 flex items-center justify-between flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <button
                            ...
                            title={`Tải Thời Khóa Biểu của ${teacher.name}`}
                          >
                            <Calendar className="w-3 h-3 text-stone-700" />
                            <span>{downloadingTeacher === `${teacher.id}-tkb` ? "Đang tạo..." : "TKB"}</span>
                          </button>
                          ...
                          >
                            ...
                            <span>{downloadingTeacher === `${teacher.id}-lbg` ? "Đang tạo..." : "LBG"}</span>
                          </button>
                          ...
                          >
                            ...
                            <span>{downloadingTeacher === `${teacher.id}-khbd` ? "Đang tạo..." : "KHBD"}</span>
                          </button>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-stone-600 mt-1 leading-snug">
                              Môn chuyên: <strong className="text-black font-semibold">{teacher.specialistSubject}</strong> • Phụ trách: {teacher.assignedClasses?.join(", ")} ({teacher.teachingPeriods} tiết/tuần)
                            </p>
                          </div>
                        </div>
                      </div>
