      ...
      <div className="bg-white p-4 border border-black shadow-[2px_2px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 md:pb-0">
          ...
          {DAYS_OF_WEEK.map((day) => {
            const dItem = scheduleItems.find((it) => it.day === day);
            const dateDisplay = dItem?.dateStr ? dItem.dateStr.slice(0, 5) : "";
            return (
              <button
                key={day}
                onClick={() => setSelectedDay(day)}
                className={`px-2.5 py-1.5 text-xs font-bold uppercase tracking-wider border border-black transition-colors whitespace-nowrap ${
                  selectedDay === day
                    ? "bg-black text-white"
                    : "bg-white text-stone-800 hover:bg-stone-200"
                }`}
              >
                {day} {dateDisplay ? `(${dateDisplay})` : ""}
              </button>
            );
          })}
        </div>
