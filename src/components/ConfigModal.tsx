...
import { X, Save, User, School, Calendar, Type, Building2, BookOpen, Layers, CheckSquare, Square, Award, Clock } from "lucide-react";
import { DEFAULT_TEACHERS, DEFAULT_CLASSES } from "../data/defaultTimetables";
import { getWeekDateRange, getAll35Weeks, detectCurrentSchoolWeek } from "../utils/dateHelper";
      ...
      <div className="bg-white border-2 border-black shadow-[6px_6px_0px_rgba(0,0,0,1)] w-full max-w-3xl overflow-hidden font-serif animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[82vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  ...
                  className="text-[10px] uppercase font-bold text-blue-800 hover:text-blue-950 flex items-center gap-1 cursor-pointer"
                  title="Tự động cập nhật theo ngày hôm nay"
                >
                  <Clock className="w-3 h-3" />
                  <span>Hôm nay (Tự động)</span>
                </button>
              </div>
              <select
                value={formData.week}
                onChange={(e) => {
                  ...
                  const range = getWeekDateRange(w, "07/09/2026");
                  setFormData({
                    ...formData,
                    week: w,
                ...
                className="w-full px-3 py-2 text-xs border border-black bg-stone-50 focus:outline-none font-bold cursor-pointer"
              >
                ...
                <optgroup label="-- 🌸 NGHỈ TẾT (25/01/2027 - 07/02/2027) --">
                  <option disabled value={999}>
                    🌸 Nghỉ Tết Nguyên Đán 2 tuần (Đã bù tự động)
                  </option>
                </optgroup>
                <optgroup label="-- HỌC KỲ II (Tuần 21 -> Tuần 35) [Sau 2 tuần Tết] --">
            ...
            {/* Date range */}
            <div>
              <label className="block text-[10px] uppercase font-bold tracking-wider text-black mb-1">Từ ngày (Thứ 2):</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[10px] uppercase font-bold tracking-wider text-black mb-1">Đến ngày (Thứ 6):</label>
              <input
                type="text"
