...
import { WeekendQuizBar } from "./components/WeekendQuizBar";
import { WeekendQuizModal } from "./components/WeekendQuizModal";
...
export function App() {
  ...
  const [schoolInfo, setSchoolInfo] = useState<SchoolInfo>(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.version === "2026_tan_hoa_a_v3_calendar35_tet_sync") {
          return parsed;
        }
      } catch (e) {}
    }
    return {
      version: "2026_tan_hoa_a_v3_calendar35_tet_sync",
      teacherName: "Cô Oanh",
      teacherType: "homeroom" as TeacherType,
      specialistSubject: "Tiếng Anh",
      assignedClasses: ["1A1"],
      schoolName: "Trường Tiểu Học Tân Thạnh (Phân hiệu Tân Hoà A)",
      branchName: "Phân hiệu Tân Hoà A",
  ...
  const [activeTab, setActiveTab] = useState<"timetable" | "schedule" | "lessonPlan" | "syncHub" | "integration" | "quizzes">("schedule");
  ...
  const [selectedQuizSubject, setSelectedQuizSubject] = useState<string | undefined>(undefined);
  ...
  const handleUpdateSchoolInfo = (newInfo: SchoolInfo) => {
    ...
    refreshScheduleAndPlans(masterTimetable, newInfo);
  };
  ...
  return (
    <div className="min-h-screen bg-[#fdfdfc] flex flex-col font-sans text-[#1a1a1a] antialiased selection:bg-black selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-5">
        {/* Thanh Tải Các Phiếu Bài Tập Trắc Nghiệm Cuối Tuần Theo LBG & KHBD (Loigiaihay.com) */}
        <WeekendQuizBar
          schoolInfo={schoolInfo}
          onOpenQuizModal={(subject) => {
            setSelectedQuizSubject(subject);
        ...
        <div className="mb-6 p-3.5 bg-white border-2 border-black shadow-[3px_3px_0px_rgba(0,0,0,1)] flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              ...
              title="Mở Bảng Đồng Bộ 18 Giáo viên toàn trường: TKB - LBG - KHBD"
            >
              <Users className="w-3.5 h-3.5" />
              <span>Đồng Bộ 18 Giáo Viên</span>
            </button>
        ...
        {activeTab === "quizzes" && (
          <div className="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_rgba(0,0,0,1)]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-black">
              <div>
                <span className="text-[10px] font-mono font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 px-2 py-0.5 uppercase">
                  Loigiaihay.com • Khối {schoolInfo.grade} • Tuần {schoolInfo.week} ({schoolInfo.startDate} - {schoolInfo.endDate})
                </span>
                <h2 className="text-xl sm:text-2xl font-serif font-black uppercase text-black mt-2">
                  Phiếu Bài Tập Trắc Nghiệm Cuối Tuần (Theo Lịch Báo Giảng KHBD)
                </h2>
                <p className="text-xs text-stone-600 mt-1 font-serif">
                  Phiếu bài tập trắc nghiệm cuối tuần định dạng Word A4 (.docx) và làm bài trực tuyến. Khớp với nội dung bài dạy Tuần {schoolInfo.week} môn:{" "}
                  <strong className="text-black">
                    {schoolInfo.grade <= 3
                      ? "Toán, Tiếng Việt, Đạo đức, HĐTN, TNXH"
                      : "Toán, Tiếng Việt, Đạo đức, HĐTN, Khoa học, Lịch sử và Địa lí"}
                  </strong>.
                </p>
              </div>
              <button
                type="button"
                onClick={() => {
                  setSelectedQuizSubject(undefined);
                  setIsQuizModalOpen(true);
                }}
                className="px-4 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs uppercase tracking-wider border border-black shadow-[2px_2px_0px_rgba(0,0,0,1)] cursor-pointer shrink-0"
              >
                Làm Trắc Nghiệm Trực Tuyến & Tải Word
              </button>
            </div>
      ...
      {/* Modal: Interactive Weekend Quiz Practice & Word Exporter (Loigiaihay) */}
      <WeekendQuizModal
